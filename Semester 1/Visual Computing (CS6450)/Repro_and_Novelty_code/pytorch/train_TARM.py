...
parser.add_argument('--work_dir', default='LM-TFM_TARM_0.2', type=str,
...
if args.restart:
    print(os.path.join(args.restart_dir, 'model.pt'))
...
def train():
    ...
    for batch, (data, target, seq_len) in enumerate(train_iter):
        ...
        # moving average of mem tokens #FIXME Implementation Not completed yet
        # raise NotImplementedError("moving average of mem tokens not implemented fully yet")
    
        if mem_tokens_ema is None:
            mem_tokens_ema = mem_tokens

        mem_tokens_ema = mu * mem_tokens_ema + (1-mu) * mem_tokens
        mem_tokens = mem_tokens_ema.detach()
